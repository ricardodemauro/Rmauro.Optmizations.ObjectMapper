    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        context.RegisterPostInitializationOutput(x =>
        {
            x.AddSource(
                ...
                SourceText.From(AttributeClass,
                Encoding.UTF8));
        });

        IncrementalValuesProvider<Class2Generate?> classes2Generate = context.SyntaxProvider
        ...
        context.RegisterSourceOutput(classes2Generate,
            static (spc, source) => Execute(source, spc));
    }

    static Class2Generate? GetClass2Generate(SemanticModel semanticModel, SyntaxNode classDeclarationSyntax)
    {
        ...
        string @namespace = classFullName[..^(className.Length + 1)];

        // Get all the members in the enum
        ImmutableArray<ISymbol> classMembers = symbol.GetMembers();
...
{{
    ...
    {{");
        ...
        sb.Append($@"
        [System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.AggressiveInlining)]
        public static void MapTo({class2Generate.ClassName} source, {class2Generate.ClassName} target)
        {{");
